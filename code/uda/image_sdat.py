...
def train_source(args):
    ...
    if args.net[0:3] == 'res':
        #netF = network.ResBase2(res_name=args.net).cuda()
    ...
    #model_list = nn.ModuleList([netF, netB, netC])
    #params = nn.ParameterList([p for model in model_list for p in model.parameters()])    
...
#                                  pool_layer=pool_layer, finetune=True).cuda()
    ...
    summary(netB,(1,2048,7,7))
    summary(netC,(1,2048))
    
    #print (classifier_feature_dim,num_classes)
...
#     summary(netC,(1,2048))
    ...
    while iter_num < max_iter:
        ...
        #y, f = classifier(x)
        f = netB(netF(x))  # Get features from netB
        y = netC(f)   # Get class logits from netC
        ...
        #cls_loss = CrossEntropyLabelSmooth(num_classes=args.class_num, epsilon=args.smooth)(y_s, labels_source)            
...
#         print (optimizer.param_groups[0]['lr'],optimizer.param_groups[161]['lr'],optimizer.param_groups[-1]['lr'])
